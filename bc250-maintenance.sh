...
component_has_artifacts() {
    case "$1" in
        ...
        cec)
            ...
            ;;
        ac3)
            [[ -e "$HOME/.config/wireplumber/wireplumber.conf.d/90-bc250-hdmi-ac3.conf" \
                || -e /etc/udev/rules.d/91-bc250-hdmi-ac3.rules \
                || -e /etc/atomic-update.conf.d/bc250-ac3.conf ]]
            ;;
        power)
